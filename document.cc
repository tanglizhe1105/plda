...
LDADocument::LDADocument(const DocumentWordTopicsPB& topics,
                         int num_topics, int inputDocumentId) {
  ...
  this->documentId= inputDocumentId;
}
...
void LDADocument::ResetWordIndex(const map<string, int>& word_index_map) {
  for (int i = 0; i < topic_assignments_->words_.size(); ++i) {
	...
	void outputPLDAPLUSCorpus(PLDAPLUSCorpus * t, std::ofstream& out){
		LDACorpus* pCorpus = t->corpus; //list<LDADocument*> LDACorpus;
		LDACorpus  corpus = *pCorpus;
		
		for(LDACorpus::iterator it= corpus.begin();  it!=corpus.end(); it++){
			LDADocument * ldaDocument = *it;
			const vector<int64>& topic_distribution = ldaDocument->topic_distribution();
			out << ldaDocument->documentId << " : ";
			for(int i = 0 ; i<topic_distribution.size(); i++){
				out<<topic_distribution[i]<<" ";
			}
            out << std::endl;
		}
	}
